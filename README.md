...
| PR or diff needing review | developer | `pr-review` | lite | reviews intent, architecture, correctness, tests, and feedback; reviewing a target PR URL posts one comment-only GitHub review unless explicitly read-only, draft-only, report-only, or no-posting |
...
- `pr-review` — context → architecture → line-level review → optional local test → published feedback → human signoff.
  - Use when: a PR/diff exists and needs correctness, safety, maintainability, or test review. Reviewing a target PR URL defaults to posting one comment-only GitHub review unless the user explicitly requests read-only, draft-only, report-only, or no posting.
